@echo off
REM ========================================
REM HonyRun Project Stop Script - Batch Version
...
REM @version: 1.0.3
REM Description: Stop MySQL, Redis and Spring Boot application services
REM Note: This script calls the corresponding PowerShell script for unified maintenance
REM ========================================

echo [%date% %time%] [INFO] Calling PowerShell stop script...
echo [%date% %time%] [INFO] Script path: %~dp0stop.ps1

REM Check if PowerShell script exists
if not exist "%~dp0stop.ps1" (
    echo [%date% %time%] [ERROR] PowerShell stop script not found: %~dp0stop.ps1
    echo [%date% %time%] [ERROR] Please ensure stop.ps1 file exists in the same directory
    pause
    exit /b 1
)
